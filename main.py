...
class AzlaMain(BoxLayout):
    ...
    start_button = Button(text='Start',
                          background_color=gcfh(Colors.bg),
                          font_size=Size.normal)
    exit_button = Button(text='Exit',  background_color=gcfh(
        Colors.bg), font_size=Size.normal)
    next_button = Button(text='Next', size_hint=(0.3, 0.05), pos_hint={
                         'center_x': 0.5, 'center_y': 1}, background_color=gcfh(Colors.bg))
    back_button = Button(text='Back', background_color=gcfh(Colors.bg))
    result_button = Button(text='Show Result', size_hint=(0.3, 0.05), pos_hint={
                           'center_x': 0.5, 'center_y': 1}, background_color=gcfh(Colors.bg))
    submit_button = Button(text='Submit', size_hint=(0.3, 0.1), pos_hint={
                           'center_x': 0.5, 'center_y': 1}, background_color=gcfh(Colors.bg))
    setting_button = Button(text='Setting', background_color=gcfh(
        Colors.bg), font_size=Size.normal)
    ...
    correct_label = Label(text='', size_hint=(0.5, 0.1), pos_hint={
                          'center_x': 0.5, 'center_y': 0.1}, color=gcfh(Colors.correct), font_size=Size.normal)
    incorrect_label = Label(text='', size_hint=(0.5, 0.1), pos_hint={
                            'center_x': 0.5, 'center_y': 0.1}, color=gcfh(Colors.incorrect), font_size=Size.normal)
    end_label = Label(text='You have reached the end', size_hint=(0.5, 0.1), pos_hint={
                      'center_x': 0.5, 'center_y': 0.1}, color=gcfh(Colors.incorrect), font_size=Size.normal)
    end_label_correct = Label(text='', size_hint=(0.5, 0.1), pos_hint={
                              'center_x': 0.5, 'center_y': 0.1}, color=gcfh(Colors.incorrect), font_size=Size.normal)
    end_label_incorrect = Label(text='', size_hint=(0.5, 0.1), pos_hint={
                                'center_x': 0.5, 'center_y': 0.1}, color=gcfh(Colors.incorrect), font_size=Size.normal)

    # Create seperator widgets
    seperator = Label(text='', size_hint=(0.5, 0.1), pos_hint={
                      'center_x': 0.5, 'center_y': 0.1})
    seperator2 = Label(text='', size_hint=(0.5, 0.1), pos_hint={
                       'center_x': 0.5, 'center_y': 0.1})
    seperator3 = Label(text='', size_hint=(0.5, 0.1), pos_hint={
                       'center_x': 0.5, 'center_y': 0.1})
    ...
    sec_image = Image(source='images/flag.jpg', size_hint=(0.5, 0.1),
    ...
    def __init__(self, **kwargs):
        ...
        Window.clearcolor = gcfh(Colors.bg)
        # Create drop down button
        self.dropdown_word = Button(text='Choose WordList',
                                    on_release=self.open_dropdown_word,
                                    background_color=gcfh(Colors.bg),
                                    font_size=Size.normal)
        ...
        for key, value in option_widgets.items():
            value.background_color = (gcfh(Colors.bg))
            value.color = (gcfh(Colors.label))
        ...
        for color_name, color_value in vars(Colors).items():
            if not color_name.startswith("__"):  # Exclude built-in attributes
                label = Label(text=color_name, size_hint=(None, None),
                              size=(100, 30),
                              pos_hint={'center_x': 0.5, 'center_y': 0.1})
                label.color = (gcfh(Colors.label))
                label.font_size = Size.normal
                entry = TextInput(multiline=False,
                                  font_size=Size.normal,
                                  border=(2, 2, 2, 2), size=(100, 30))
    ...
    def select_word(self, item_text):
        ...
        Default.labels_widgets, Default.correct, Default.word = Question.widget(
            Default.choice)
    ...
    def next_button_pressed(self, instance):
        ...
        for index, (key, value) in enumerate(Default.labels_widgets.items()):
            if index == self.count:
                ...
                # If you have reached the last question then
                # it will show no entry box and buttons
                if value is None:
                    self.grid_layout_question.remove_widget(self.entry_box)
                    self.grid_layout_question.remove_widget(self.submit_button)
                    self.grid_layout_question.remove_widget(self.seperator2)
    ...
    def result_button_pressed(self, instance):
        ...
        self.end_label_correct.color = gcfh(Colors.label)
