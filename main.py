...
class StartScreen(BoxLayout):
    ...
    back_button = Button(text='Back', size_hint=(0.3, 0.05), pos_hint={'center_x': 0.5, 'center_y': 1}, background_color=get_color_from_hex(Colors.bg))
    ...
    entry_box = TextInput(multiline=False, size_hint=(0.5, 0.1), pos_hint={'center_x': 0.5, 'center_y': 0.1},
                          font_size = Size.normal, border = (2, 2, 2, 2))
    ...
    sec_image = Image(source='images/flag.jpg')

    # Create grid layout
    grid_layout = GridLayout(cols=1, spacing = 0)
    grid_layout_main = GridLayout(cols=1, spacing = 5)
    grid_layout_question = GridLayout(cols=1, spacing = 5)
    ...
    def __init__(self, **kwargs):
        ...
        for color_name, color_value in vars(Colors).items():
            if not color_name.startswith("__"):  # Exclude built-in attributes
                ...
                label = Label(text=color_name, size_hint=(None, None), size=(100, 30))
